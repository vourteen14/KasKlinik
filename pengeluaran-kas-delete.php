...
{
  header("Location: pengeluaran-kas.php?message=" . urlencode($message));
  exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'])) {
  ...
  try {
    ...
    $stmt = $conn->prepare("SELECT total_price FROM transaction_out WHERE id = ?");
    $stmt->execute([$id]);
    $totalPrice = $stmt->fetchColumn();

    // Persiapkan dan jalankan query DELETE
    $stmt = $conn->prepare("DELETE FROM transaction_out WHERE id = ?");
    $stmt->execute([$id]);
