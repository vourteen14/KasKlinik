...
<body class="app">
	...
	<script>
		...
		function printInvoiceKeluar(index) {
			...
			printWindow.document.write('<tr><td>1</td><td>' + index.id + '</td><td>' + index.suppliers + '</td><td>' + index.information + '</td><td>' + index.total_price + '</td><td>'+ index.created_at + '</td></tr>');
			printWindow.document.write('</tbody>');
			printWindow.document.write('</table>');
		...
		function generateBilling(index) {
			...
			billingWindow.document.write('<p><span class="label">Pembayaran:</span><span class="value">&nbsp;&nbsp;&nbsp;' + index.category + '</span></p>');
